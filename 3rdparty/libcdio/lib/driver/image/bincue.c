...
_init_bincue(_img_private_t *p_env)
{
  ...
  /* layout_cue_tracks() creates the leadout entry after all source files
     have been measured. */
...
_read_bincue (void *p_user_data, void *data, size_t size)
{
  ...
  while (size > 0) {
    ...
    if (p_env->pos.lba >= p_env->tocent[p_env->pos.index+1].start_lba) {
      ...
      if (cdio_stream_seek(this_track->data_source,
                           this_track->offset + this_track->datastart,
                           SEEK_SET) != 0)
        return -1;
    }
  }
...
get_disc_last_lsn_bincue (void *p_user_data)
{
  ...
  return cdio_lba_to_lsn(p_env->tocent[p_env->gen.i_tracks].start_lba);
}

/**
  Convert the file-relative INDEX positions collected by parse_cuefile into
  disc-relative positions. CUE INDEX values restart at zero when a new FILE
  begins, so deriving the TOC directly from them only works for one-file
  images.
 */
static bool
  ...
  /* CDTEXTFILE "<filename>" */
      ...
      if (cd) {
        ...
        if(NULL == (source = cdio_stdio_new(psz_filename))) {
          cdio_log(CDIO_LOG_WARN,
                   "%s line %d: can't open optional CD-TEXT file `%s'; ignoring it",
        ...
        if (size < 5) {
          cdio_log(CDIO_LOG_WARN,
		   "%s line %d: optional file `%s' is too small to contain CD-TEXT; ignoring it",
...
const char *
cdio_bincue_get_track_filename (const CdIo_t *p_cdio, track_t i_track)
{
  ...
  return p_env->tocent[i_track - p_env->gen.i_first_track].filename;
}
                           ...
                           bool b_form2)
{
  _img_private_t *p_env = p_user_data;
  char buf[CDIO_CD_FRAMESIZE_RAW] = { 0, };
  driver_return_code_t ret = read_raw_sector_bincue(p_env, buf, lsn);
  if (ret != DRIVER_OP_SUCCESS)
    return ret;
...
cdio_open_cue (const char *psz_cue_name)
{
  ...
  if (NULL == psz_bin_name) {
    cdio_warn ("source name %s is not recognized as a CUE file",
               psz_cue_name);
  }
