...
static void cdvdCreateNewNVM(std::FILE* fp)
{
	...
	if (BiosVersion >= 0x200)
	{
		...
		std::fseek(fp, nvmLayout->regparams + 0x10, SEEK_SET);
...
static void cdvdNVM(u8* buffer, int offset, size_t bytes, bool read)
{
	...
	char filename[1024];
	snprintf(filename, sizeof(filename), "%s/%s", EmuFolders::Bios.c_str(), "ilink_id.bin");

	auto fpIlink = FileSystem::OpenManagedCFile(filename, "rb");
		...
		switch (rt)
		{
			...
			case 0x03: // Mecacon-command
				switch (cdvd.SCMDParamBuff[0])
				{
					case 0x00: // get mecha version (1:4), supported by mecha 3.9
						SetSCMDResultSize(4);
						memcpy(cdvd.SCMDResultBuff, &temp_mechaver[0], 4);
						break;
					...
					default:
						SetSCMDResultSize(1);
						cdvd.SCMDResultBuff[0] = 0x80;
						Console.Warning("*Unknown Mecacon Command param[0]=%02X", cdvd.SCMDParamBuff[0]);
						break;
				}
				break;
			...
			case 0x36: //cdvdman_call189 [__sceCdReadRegionParams - made up name] (0:15) i think it is 16, not 15
				...
				else
				{
					...
					//			cdvd.SCMDResultBuff[3] == ROMVER[4] == *0xBFC7FF04		J A E H C
					//			cdvd.SCMDResultBuff[4] == OSDVER[4] == CAP				Jjpn, Aeng, Eeng, Heng, Reng, Csch, Kkor, Htch, Aspa
					//			cdvd.SCMDResultBuff[5] == OSDVER[5] == small
					//			cdvd.SCMDResultBuff[6] == OSDVER[6] == small
					//			cdvd.SCMDResultBuff[7] == OSDVER[7] == small
					//			cdvd.SCMDResultBuff[8] == VERSTR[0x22] == *0xBFC7FF52	J A E
					//			cdvd.SCMDResultBuff[9] == DVDID							J U O E A R C M
					//			cdvd.SCMDResultBuff[10]== 0; //??
